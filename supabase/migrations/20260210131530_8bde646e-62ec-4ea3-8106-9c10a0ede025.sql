...
AS $$
  WITH main_orders AS (
    SELECT * FROM orders WHERE external_order_id NOT LIKE '%-offer%'
  ),
  latest_approved AS (
  ...
  SELECT
    ...
    (SELECT count(*) FROM main_orders) as total_orders,
    (SELECT coalesce(sum(amount), 0) FROM main_orders WHERE status = 'approved') as approved_revenue,
